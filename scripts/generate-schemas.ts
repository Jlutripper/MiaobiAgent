import { Project, Type, Symbol, Node } from 'ts-morph';
...
class TypeConverter {
    ...
    if (type.isArray()) {
        ...
        if (unionTypes.every(t => t.isStringLiteral())) {
            ...
            if (unionTypes.some(t => t.isObject())) {
                return { type: 'OBJECT', description: "Could be one of multiple object types." };
            }
    ...
    return { type: 'STRING', description: `Unsupported type: ${type.getText()}` };
    }
...
`;
        for (const typeName of typeNames) {
            ...
            if (typeAliasOrInterface) {
                ...
                output += `export const ${schemaName} = ${JSON.stringify(schema, null, 2)};\n\n`;
            } else {
