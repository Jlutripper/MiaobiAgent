                ...
                <div className="mt-4 space-y-4">
                    ...
                    <div>
                     ...
                     <div>
                        ...
                        <input type="number" value={section.flexGrow ?? 0} onChange={e => onUpdate({ flexGrow: parseFloat(e.target.value) || 0 })} className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md" />
                ...
                <div className="space-y-4">
                    <details className="bg-gray-900/50 p-3 rounded-lg border border-gray-700" open><summary className="font-semibold cursor-pointer">内容和字体</summary><div className="mt-4 space-y-4">
                        ...
                        <div className="flex gap-4">
                            <div className="flex-1"><label className="block text-sm font-medium text-gray-400 mb-1">字号</label><input type="number" value={section.style.fontSize} onChange={e => handleStyleUpdate({ fontSize: parseInt(e.target.value) || 0 })} onBlur={e => { if (!e.target.value || parseInt(e.target.value) <= 0) { handleStyleUpdate({ fontSize: 24 }); } }} className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md text-white" /></div>
                        ...
                        <div className="flex gap-4">
                            <div className="flex-1"><label className="block text-sm font-medium text-gray-400 mb-1">行间距</label><input type="number" step="0.1" value={section.style.lineHeight} onChange={e => handleStyleUpdate({ lineHeight: parseFloat(e.target.value) || 1.5 })} disabled={!!section.style.curve && section.style.curve !== 0} className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md text-white disabled:opacity-50" /></div>
                            <div className="flex-1"><label className="block text-sm font-medium text-gray-400 mb-1">字间距 (px)</label><input type="number" value={section.style.letterSpacing || 0} onChange={e => handleStyleUpdate({ letterSpacing: parseInt(e.target.value) || 0 })} className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md text-white" /></div>
                        </div>
                ...
                <div className="space-y-4">
                    ...
                    {(isTemplateMode) && (
                        <div className="grid grid-cols-2 gap-2">
                            ...
                            <div><label className="block text-sm font-medium text-gray-300 mb-1">区块高度 (px)</label><input type="number" value={section.height || ''} onChange={e => onUpdate({ height: parseInt(e.target.value) || undefined })} placeholder="自动" className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md" /></div>
                        </div>
                    )}
                     <div><label className="block text-sm font-medium text-gray-300 mb-1">旋转 (°)</label><input type="number" value={section.rotation || 0} onChange={e => onUpdate({ rotation: parseInt(e.target.value) || 0 })} className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md" /></div>
