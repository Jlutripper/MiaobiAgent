...
const GlobalPanel = ({ template, onUpdate, onFileUpload, isProcessing }: { template: PosterTemplate, onUpdate: (updates: Partial<PosterTemplate>) => void, onFileUpload: any, isProcessing: boolean }) => (
    <div className="space-y-4">
        <details className="bg-gray-900/50 p-3 rounded-lg border border-gray-700" open><summary className="font-semibold cursor-pointer">模板信息</summary><div className="mt-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
                <div>
                    ...
                    <input type="number" value={template.width || ''} onChange={e => onUpdate({ width: parseInt(e.target.value) || 0 })} placeholder="必需" className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1">画布高度 (px)*</label>
                    <input type="number" value={template.height || ''} onChange={e => onUpdate({ height: parseInt(e.target.value) || 0 })} placeholder="必需" className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md" />
                </div>
            </div>
        ...
        <details className="bg-gray-900/50 p-3 rounded-lg border border-gray-700" open><summary className="font-semibold cursor-pointer">页面背景</summary><div className="mt-4 space-y-4">
            <ColorSwatch label="背景颜色/渐变" value={template.background.value} onChange={v => onUpdate({ background: { ...template.background, value: v } })} />
            <button onClick={() => { const i = document.createElement('input'); i.type = 'file'; i.accept = 'image/*'; i.onchange = e => onFileUpload((e.target as HTMLInputElement).files?.[0], { maxWidth: 1280, quality: 0.8 }, (b64:string) => onUpdate({ background: {...template.background, type: 'image', value: b64} })); i.click(); }} disabled={isProcessing} className="w-full flex items-center justify-center gap-2 p-2 bg-purple-600 hover:bg-purple-700 rounded-md text-sm disabled:bg-gray-500">{isProcessing ? <SpinnerIcon className="w-5 h-5"/> : <ArrowUpTrayIcon className="w-5 h-5"/>} 上传图片</button>
...
const FlexItemPanel = ({ section, onUpdate }: { section: ArticleSection, onUpdate: (updates: Partial<ArticleSection>) => void }) => (
    <>
        <div>
            ...
            <input type="number" min="0" value={section.flexGrow ?? 0} onChange={e => onUpdate({ flexGrow: parseFloat(e.target.value) || 0 })} className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md" />
        </div>
         <div>
            <label className="flex items-center gap-1 text-sm font-medium text-gray-300 mb-1">
                空间压缩权重 (Shrink)
            ...
            <input type="number" min="0" value={section.flexShrink ?? 1} onChange={e => onUpdate({ flexShrink: parseFloat(e.target.value) || 0 })} className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md" />
        </div>
    </>
);
        ...
        <div className="space-y-4">
            ...
            {parentIsGrid ? (
                ...
                <details className="bg-gray-900/50 p-3 rounded-lg border border-gray-700" open><summary className="font-semibold cursor-pointer">位置与尺寸</summary><div className="mt-4 space-y-4">
                    ...
                    <div><label className="block text-sm font-medium text-gray-300 mb-1 mt-3">图层顺序 (Z-Index)</label><input type="number" value={box.zIndex || 0} onChange={e => onUpdate({ zIndex: parseInt(e.target.value, 10) || 0 })} className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md" /></div>
                </div></details>
            )}
            ...
            <details className="bg-gray-900/50 p-3 rounded-lg border border-gray-700"><summary className="font-semibold cursor-pointer">内容布局</summary><div className="mt-4 space-y-4">
                 ...
                 {box.layoutMode === 'grid' ? (
                     <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                             <div><label className="text-sm">列间距</label><input type="number" value={box.columnGap || 0} onChange={e => onUpdate({ columnGap: parseInt(e.target.value, 10) || 0 })} className="w-full p-1 bg-gray-700 rounded mt-1"/></div>
                             <div><label className="text-sm">行间距</label><input type="number" value={box.rowGap || 0} onChange={e => onUpdate({ rowGap: parseInt(e.target.value, 10) || 0 })} className="w-full p-1 bg-gray-700 rounded mt-1"/></div>
                        </div>
                     </div>
                 ) : (
            ...
            <details className="bg-gray-900/50 p-3 rounded-lg border border-gray-700"><summary className="font-semibold cursor-pointer">外观</summary><div className="mt-4 space-y-4">
                ...
                <div><label className="block text-sm font-medium text-gray-300 mb-1">圆角 (px)</label><input type="number" value={box.borderRadius} onChange={e => onUpdate({ borderRadius: parseInt(e.target.value, 10) || 0 })} className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md" /></div>
                <div><label className="block text-sm font-medium text-gray-300 mb-1">内边距 (px)</label><div className="grid grid-cols-2 gap-2">
                    <input type="number" value={box.paddingTop} onChange={e => onUpdate({ paddingTop: parseInt(e.target.value, 10) || 0})} placeholder="上" className="w-full p-1 bg-gray-700 border border-gray-600 rounded-md" />
                    <input type="number" value={box.paddingBottom} onChange={e => onUpdate({ paddingBottom: parseInt(e.target.value, 10) || 0})} placeholder="下" className="w-full p-1 bg-gray-700 border border-gray-600 rounded-md" />
                    <input type="number" value={box.paddingLeft} onChange={e => onUpdate({ paddingLeft: parseInt(e.target.value, 10) || 0})} placeholder="左" className="w-full p-1 bg-gray-700 border border-gray-600 rounded-md" />
                    <input type="number" value={box.paddingRight} onChange={e => onUpdate({ paddingRight: parseInt(e.target.value, 10) || 0})} placeholder="右" className="w-full p-1 bg-gray-700 border border-gray-600 rounded-md" />
                </div></div>
            </div></details>
        </div>
    );
                ...
                <div><label className="flex items-center gap-2 text-sm font-medium text-gray-300 mb-2"><input type="checkbox" checked={!!section.style.textShadow} onChange={() => handleStyleUpdate({ textShadow: section.style.textShadow ? undefined : '2px 2px 4px rgba(0,0,0,0.5)' })} className="h-4 w-4 rounded bg-gray-600 text-purple-600 focus:ring-purple-500 border-gray-500"/>阴影</label>
                    {section.style.textShadow && <div className="space-y-2 pl-6">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            <input type="number" value={shadowOffsetX} onChange={e => handleTextShadowChange('offsetX', parseInt(e.target.value, 10))} className="w-full p-1 bg-gray-600 rounded-md" />
