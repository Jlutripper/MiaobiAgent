...
export const createChat = (customTools: CustomTool[], longArticleTemplates: LongArticleTemplate[], posterTemplates: PosterTemplate[]): Chat => {
...
export const getChatResponse = async (chat: Chat, prompt: string): Promise<{ tool: Tool; prompt?: string; displayPrompt?: string; reply: string; initialContent?: string; templateId?: string; }> => {
    const response = await chat.sendMessage({ message: prompt });
    try {
        const result = JSON.parse(response.text);
        // Ensure prompt is not undefined, which is required by the schema but sometimes omitted by the model
        if (!result.prompt) {
            result.prompt = prompt;
        }
        return result;
    } catch (e) {
        console.error("Failed to parse routing response:", response.text);
        // Fallback for when the model doesn't return valid JSON
        return { tool: 'chat', reply: response.text || "对不起，我没太理解。可以换一种方式问我吗？", prompt: prompt };
    }
};
