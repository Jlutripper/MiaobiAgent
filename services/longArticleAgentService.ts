    ...
    if (template) {
        ...
        if (textRoles.length > 0) {
            contentSchemaProperties.textContent = {
                ...
                properties: textRoles.reduce((acc, role) => ({ ...acc, [role]: { type: Type.STRING, description: `Content for the '${role}' text block.` } }), {}),
                required: textRoles
            };
        ...
        if (imageRoles.length > 0 && generateIllustrations) {
            contentSchemaProperties.imagePrompts = {
                ...
                properties: imageRoles.reduce((acc, role) => ({ ...acc, [role]: { type: Type.STRING, description: `Image prompt for the '${role}' image block.` } }), {}),
                required: imageRoles,
            };
        }
...
5.  **JSON OUTPUT:** Respond ONLY with a valid JSON object adhering to the schema. Do not include locked roles in your output.`;
        ...
        const structuredContent = JSON.parse(contentResponse.text);
        ...
        if (generateIllustrations && structuredContent.imagePrompts && imageRoles.length > 0) {
            const imageGenerationPromises = imageRoles.map(async (role) => {
                ...
                try {
                    const illustrationResponse = await ai.models.generateImages({
                        model: 'imagen-3.0-generate-002',
                        prompt: prompt,
                        config: { numberOfImages: 1, outputMimeType: 'image/png', aspectRatio: '1:1' }
                    });
                    const imageUrl = `data:image/png;base64,${illustrationResponse.generatedImages[0].image.imageBytes}`;
...
5.  Respond with a single JSON object.`;
        ...
        const layoutData = JSON.parse(contentResponse.text);
